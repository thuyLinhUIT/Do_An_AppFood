...
export default function RecipeDetailScreen(props) {
    ...
    const updateFavourite = async () => {
        try {
            ...
            const response = await axios
                .post("http://192.168.1.198:3000/api/UpdateFavourite", {
    ...
    //Xóa 1 id Favourite xuống csdl
    const delete1favourite = async () => {
        try {
            var token = await AsyncStorage.getItem("token");
            var valueToken = JSON.parse(token).token;
            const response = await axios
                .delete(`http://192.168.1.198:3000/api/Delete1favourite/${item.idMeal}`,
                    {
                        headers: {
    ...
    const sendComment = async () => {
        try {
            ...
            const response = await axios
                .post(`http://192.168.1.198:3000/api/sendComment/`, {
                    idMeal: item.idMeal,
                    idUser: user._id,
                    avatar: user.avatar,
    ...
    const getComment = async () => {
        try {
            ...
            const response = await axios.get(`http://192.168.1.198:3000/api/getComment/${item.idMeal}`);
